...
WHERE (GETDATE() NOT BETWEEN bookings.check_in_date AND bookings.check_out_date)
  AND availability = 1
...
WHERE customers.first_name = N'John'
   OR customers.email = N'johndoe@gmail.com';
...
WHERE room_id IN
      (SELECT room_id
       FROM bookings
       WHERE '2024-05-15' NOT BETWEEN bookings.check_in_date AND bookings.check_out_date);

SELECT *
FROM rooms
WHERE (rooms.room_id NOT IN (SELECT room_id FROM bookings))
   OR (rooms.room_id IN
       (SELECT room_id
        FROM bookings
